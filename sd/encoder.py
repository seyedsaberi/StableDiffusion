...
class VAE_Encoder_large(nn.Sequential):
    def __init__(self):
        super().__init__(
...
class VAE_Encoder(nn.Sequential):
    def __init__(self):
        super().__init__(
            ...
            # (Batch_size, 512, Width/4, Height/4) -> (Batch_size, 512, Width/4, Height/4)
            nn.SiLU(),
            # (Batch_size, 512, Width/4, Height/4) -> (Batch_size, 8, Width/4, Height/4)
            nn.Conv2d(512, 8, kernel_size = 3, padding = 1),
            # (Batch_size, 8, Width/4, Height/4) -> (Batch_size, 8, Width/4, Height/4)
            nn.Conv2d(8, 8 , kernel_size = 1, padding = 0),
        )
    def forward(self, x : torch.Tensor, noise: torch.Tensor) -> torch.Tensor:
        # x: (Batch_size, Channel, Width, Height)
        ...
        for name, module in self.named_children():
            if getattr(module, 'stride', None) == (2, 2):
                ...
                x = F.pad(x, (0, 1, 0, 1))
            x = module(x)
        # (Batch_size, 8, Width/4, Height/4) -> (Batch_size, 4, Width/4, Height/4), (Batch_size, 4, Width/4, Height/4)
        mean, log_variance = torch.chunk(x, 2, dim = 1)
